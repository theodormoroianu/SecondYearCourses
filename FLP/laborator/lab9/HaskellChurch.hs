...
cS :: CNat -> CNat
cS x = CNat $ \s z -> s (cFor x s z)

--Addition of m and n is done by iterating s n times over m
(+:) :: CNat -> CNat -> CNat
a +: b = CNat $ \s z -> cFor a s (cFor b s z)
infixl 6 +:
