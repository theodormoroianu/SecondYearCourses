...
int main()
{
    ...
    while (!open.empty()) {
        ...
        if (node == destination_node) {
            cout << space << "Am ajuns la nodul destinatie (" << destination_node << "), asa ca oprim cautarea!\n";
            break;
        }

        cout << space << "Nodul pe care il consideram este " << node << ", cu g:" << g << ", si f:" << f << ".\n" + space + "Incercam sa ii pargurgem vecinii:\n";
        
        for (auto [vec, cost] : edges[node]) {
            auto draw_edge = [&]() {
                out << node << "  ->  " << vec << "  g=" << g + cost << ", f=" << g + cost + estimations[vec] << "\n";
            };
            cout << space + space << "Ne uitam la nodul " << vec << ", pentru ca avem muchia " << node << "->" << vec << " de cost " << cost << ":\n";
            ...
            if (is_in_closed && cmp(potential_state, closed[poz])) {
                ...
                cout << space + space + space << vec << " este in Closed, dar cu un cost mai prost, asa ca il mutam in Open.\n";
